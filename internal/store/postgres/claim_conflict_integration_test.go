...
// Under a blind `= EXCLUDED.x`, one such write would silently drop a scope set
...
func TestPostgres_PartialRewriteKeepsScopeProvenanceAndVisibility(t *testing.T) {
	conn := withConn(t)
...
// that exists.
func TestPostgres_ExplicitRewriteOverridesScopeAndVisibility(t *testing.T) {
	conn := withConn(t)
	ctx := context.Background()
	at := time.Date(2026, 7, 1, 12, 0, 0, 0, time.UTC)
...
//
// The columns arrive by ALTER TABLE ... ADD COLUMN with a default, which is
// metadata-only on Postgres 11+ (ADR 0025 measured 23 ms over 88k rows) and
// rewrites nothing. The question this test settles is not cost but MEANING:
// what does a pre-existing row now claim about itself? Empty provenance and a
// zero authority/citation count are the truthful readings of "unknown", a NULL
// last_executed is the never-executed sentinel, and 'team' is exactly what
// query.admission already coerced an absent audience to — so no historical row
// changes meaning by acquiring the column.
//
// The INSERT below names only the pre-#338 columns, which is precisely what an
// upgraded row looks like.
func TestPostgres_RowsPredatingTheColumnsReadAsUnknown(t *testing.T) {
	conn := withConn(t)
	ctx := context.Background()
...
VALUES ($1, $2, 'fact', 0.7, 'active', now(), '<system>')`,
