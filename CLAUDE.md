...
### Hand-maintained lists — the source-drift guards (`internal/sourceguard`)

**A list a human must remember to update, next to code that never reads it, is drift waiting to happen — and it never fails a test, because nothing depends on the list being right.** Same defect class as the projection drift above, one layer up. Two had already drifted when the guards were written:

- **MCP executor keys.** A tool needs three separate mentions to work: an `mcpTools()` action, a `dispatchAxiTool(..., "<tool>", ...)` call site, and an `exec.<tool>` key in `mcpExecutorMap()` — connected by nothing but the string. The v0.85.0 brain-native rename renamed the tools and the actions and *not* the executor keys, so `list_beliefs`, `list_dissonances` and `remember_episode` (a write) dispatched to unregistered executors and returned `-32603` in every release for three weeks (#341, fixed by #343). Everything else stayed green: they were registered actions, they listed in `tools/list`, they carried the right scopes.
- **`commands` in `cmd/mnemos/ux.go`**, which drives typo correction. Six dispatched commands had drifted out of it, `health` among them, so `mnemos helth` suggested nothing.

`cmd/mnemos/axikernel_executor_guard_test.go` and `cmd/mnemos/cli_commands_guard_test.go` make both a test failure, in **both** directions — the orphan direction is what identifies the cause, since three missing executors *plus* three orphaned ones is a half-finished rename where three missing and none orphaned would be an omission. Exclusions go in a per-guard allowlist with a reason, and a satisfied entry fails as stale, exactly as above.

Extraction is **AST-based, never regex** (`internal/sourceguard`), and that is load-bearing: `case "verify", "reconsolidate":` in `main.go` is a multi-value clause, and a `case "..."` regex reads only its first value and then reports `reconsolidate` as a command that does not exist. A guard that invents drift gets disabled by the first person it costs an hour. For the same reason both extractors error rather than return a short list when they meet something they cannot read (a non-literal case, a computed tool name, no call sites at all): a construct the guard skips silently is a construct drift can hide in.
